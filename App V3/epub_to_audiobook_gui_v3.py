"""
epub_to_audiobook_gui_v3.py — EPUB → MP3 Audiobook Converter (GUI, V3)
======================================================================
Everything the V2 app does, plus the V3 upgrades:

  • DARK THEME — a Light/Dark toggle in the header; your choice (and your
    read-along preferences) are remembered between sessions.
  • ACCURATE READ-ALONG — sentence highlighting now comes straight from
    edge-tts's own timing, so it stays in sync to the end of long parts
    (the late-track drift in V2 is gone). Built on `audiobook_engine_v3`.
  • PAGE-TURN AUTO-SCROLL — the highlight is left in place until it reaches the
    bottom of the page, then the view jumps so that line becomes the second line
    from the top — a fresh page of upcoming text, instead of scrolling on every
    sentence.
  • READ-ALONG CONTROLS — reading speed (pitch-preserving), font family, font
    size, line spacing, and highlight colour.
  • EXTRAS — auto-advance through parts/chapters, resume where you left off,
    a sleep timer, and keyboard shortcuts (Space = play/pause, ←/→ = previous/
    next sentence, ↑/↓ = previous/next track).
...
Run it:  python epub_to_audiobook_gui_v3.py
"""
...
import audiobook_engine_v3 as eng
...
THEMES = {
    "light": {
        ...
        "ACCENT": "#2d6cdf", "ACCENT2": "#1f57c3", "BORDER": "#e3e6ea",
        "OK": "#137a3f", "ERR": "#b42318", "FIELD": "#ffffff",
        "BTN": "#e9ebef", "BTN_HOVER": "#dbdfe6",
        "HL_BG": "#fff3c4", "HL_FG": "#1f2430",
    },
    "dark": {
        "BG": "#1d1f27", "CARD": "#262a36", "TEXT": "#e7e9f0", "MUTED": "#99a0ad",
        "ACCENT": "#5b8cf0", "ACCENT2": "#4475d8", "BORDER": "#39404f",
        "OK": "#54c281", "ERR": "#e9786d", "FIELD": "#2f3442",
        "BTN": "#343b4d", "BTN_HOVER": "#414a60",
        "HL_BG": "#5a4d20", "HL_FG": "#ffe9a8",
    },
}
...
class App:
    ...
    def _apply_ttk_styles(self):
        ...
        st.configure("Section.TLabelframe.Label", background=p["CARD"],
                     foreground=p["ACCENT"], font=(UI_FAMILY, 11, "bold"))
        ...
        st.configure("Horizontal.TProgressbar", troughcolor=p["BORDER"],
                     background=p["ACCENT"], bordercolor=p["BORDER"], thickness=16)
        st.configure("TCombobox", padding=6, arrowsize=15, arrowcolor=p["TEXT"])
        st.map("TCombobox",
               fieldbackground=[("readonly", p["FIELD"]), ("!readonly", p["FIELD"])],
               foreground=[("readonly", p["TEXT"])],
               selectbackground=[("readonly", p["FIELD"])],
               selectforeground=[("readonly", p["TEXT"])])
        st.configure("TSpinbox", padding=6, arrowsize=15, fieldbackground=p["FIELD"],
                     foreground=p["TEXT"], background=p["CARD"], arrowcolor=p["TEXT"])
        st.configure("TEntry", padding=6, fieldbackground=p["FIELD"],
                     foreground=p["TEXT"], insertcolor=p["TEXT"])
        st.configure("TNotebook", background=p["BG"], borderwidth=0)
        st.configure("TNotebook.Tab", padding=(22, 12), font=(UI_FAMILY, 12),
                     background=p["BG"], foreground=p["MUTED"], borderwidth=0)
        st.map("TNotebook.Tab",
               background=[("selected", p["CARD"])],
               foreground=[("selected", p["TEXT"]), ("active", p["TEXT"])])
        st.configure("TScale", background=p["BG"])
    ...
    def _build_ui(self):
        self._apply_ttk_styles()
        outer = ttk.Frame(self.root, padding=(18, 14, 18, 12))
        ...
        ttk.Label(title_box, text="Convert any .epub to MP3 chapters — then read "
                  "along, in sync, as it plays.", style="Sub.TLabel").pack(anchor="w", pady=(2, 0))
        self.theme_btn = ttk.Button(
            head, text="☾ Dark" if self.theme == "light" else "☀ Light",
            width=9, command=self.toggle_theme)
        self.theme_btn.grid(row=0, column=1, sticky="ne")

        # Book picker
        src = ttk.Labelframe(outer, text=" Book ", style="Section.TLabelframe", padding=12)
        src.grid(row=1, column=0, sticky="ew", pady=(12, 0))
        ...
        ttk.Label(src, textvariable=self.info_var, style="CardMuted.TLabel",
                  wraplength=800, justify="left").grid(row=1, column=0, columnspan=3,
                                                       sticky="w", pady=(10, 0))
    ...
    # ── Convert tab (mirrors V2) ─────────────────────────────────────────────
    def _build_convert_tab(self, parent):
        parent.columnconfigure(0, weight=1)
        parent.rowconfigure(1, weight=1)

        opts = ttk.Labelframe(parent, text=" Options ", style="Section.TLabelframe", padding=12)
        ...
        # Chapters
        chap = ttk.Labelframe(parent, text=" Chapters ", style="Section.TLabelframe", padding=10)
    ...
    def _build_read_tab(self, parent):
        ...
        if not HAVE_PYGAME:
            ttk.Label(parent, style="Muted.TLabel", wraplength=800, justify="left",
                      text="In-app playback needs the 'pygame' package "
                           "(pip install pygame). You can still pick a track to read "
                           "its text and open it in your media player.").grid(
        ...
        self.track_box = ttk.Combobox(pick, textvariable=self.track_var, state="readonly", values=[])
        self.track_box.grid(row=0, column=1, sticky="ew")
        self.track_box.bind("<<ComboboxSelected>>", self._on_track_selected)
        ttk.Button(pick, text="Refresh", command=self._refresh_tracks, width=9).grid(row=0, column=2, padx=(8, 0))
        ttk.Button(pick, text="Open file", command=self._open_current_track, width=10).grid(row=0, column=3, padx=(6, 0))

        # read-along controls
        ctl = ttk.Labelframe(parent, text=" Reading ", style="Section.TLabelframe", padding=10)
        ctl.grid(row=2, column=0, sticky="ew", pady=(10, 0))
        for c in (1, 3, 5):
            ctl.columnconfigure(c, weight=0)

        ttk.Label(ctl, text="Speed", style="Card.TLabel").grid(row=0, column=0, sticky="w", padx=(0, 6))
        self.speed_var = tk.StringVar(value=f"{self.speed:g}×")
        self.rspeed_box = ttk.Combobox(ctl, textvariable=self.speed_var, width=6, state="readonly",
                                       values=[f"{s:g}×" for s in SPEED_CHOICES])
        self.rspeed_box.grid(row=0, column=1, sticky="w", padx=(0, 14))
        self.rspeed_box.bind("<<ComboboxSelected>>", self._on_read_speed)

        ttk.Label(ctl, text="Font", style="Card.TLabel").grid(row=0, column=2, sticky="w", padx=(0, 6))
        self.font_var = tk.StringVar(value=self.settings.get("font_family", "Georgia"))
        self.font_box = ttk.Combobox(ctl, textvariable=self.font_var, width=16, state="readonly",
                                     values=FONT_CHOICES)
        self.font_box.grid(row=0, column=3, sticky="w", padx=(0, 14))
        self.font_box.bind("<<ComboboxSelected>>", lambda e: self._restyle_reader(save=True))

        ttk.Label(ctl, text="Size", style="Card.TLabel").grid(row=0, column=4, sticky="w", padx=(0, 6))
        self.size_var = tk.IntVar(value=int(self.settings.get("font_size", 14)))
        sz = ttk.Spinbox(ctl, from_=10, to=32, width=5, textvariable=self.size_var,
                         command=lambda: self._restyle_reader(save=True))
        sz.grid(row=0, column=5, sticky="w", padx=(0, 14))
        self.size_var.trace_add("write", lambda *_: self._restyle_reader(save=True))

        ttk.Label(ctl, text="Line spacing", style="Card.TLabel").grid(row=1, column=0, sticky="w", padx=(0, 6), pady=(8, 0))
        self.ls_var = tk.IntVar(value=int(self.settings.get("line_spacing", 6)))
        ls = ttk.Spinbox(ctl, from_=0, to=50, width=5, textvariable=self.ls_var,
                         command=lambda: self._restyle_reader(save=True))
        ls.grid(row=1, column=1, sticky="w", pady=(8, 0))
        self.ls_var.trace_add("write", lambda *_: self._restyle_reader(save=True))

        ttk.Label(ctl, text="Highlight", style="Card.TLabel").grid(row=1, column=2, sticky="w", padx=(0, 6), pady=(8, 0))
        self.hl_var = tk.StringVar(value=self.settings.get("highlight") or "Amber")
        self.hl_box = ttk.Combobox(ctl, textvariable=self.hl_var, width=10, state="readonly",
                                   values=list(HL_PRESETS[self.theme].keys()) + ["Custom…"])
        self.hl_box.grid(row=1, column=3, sticky="w", pady=(8, 0))
        ...
        ttk.Checkbutton(ctl, text="Auto-advance", variable=self.adv_var, style="TCheckbutton",
                        command=self._save_settings).grid(row=1, column=4, columnspan=2, sticky="w", pady=(8, 0))

        ttk.Label(ctl, text="Sleep timer", style="Card.TLabel").grid(row=2, column=0, sticky="w", padx=(0, 6), pady=(8, 0))
        self.sleep_var = tk.StringVar(value="Off")
        self.sleep_box = ttk.Combobox(ctl, textvariable=self.sleep_var, width=12, state="readonly",
                                      values=SLEEP_CHOICES)
        self.sleep_box.grid(row=2, column=1, columnspan=2, sticky="w", pady=(8, 0))
        self.sleep_box.bind("<<ComboboxSelected>>", self._on_sleep_pick)
        self.hint_lbl = ttk.Label(ctl, style="CardMuted.TLabel",
                                  text="Space = play/pause   ←/→ = sentence   ↑/↓ = track")
        self.hint_lbl.grid(row=2, column=3, columnspan=3, sticky="w", pady=(8, 0))

        # reader
        text_wrap = self._reg(tk.Frame(parent, highlightthickness=1), bg="CARD", highlightbackground="BORDER")
        text_wrap.grid(row=3, column=0, sticky="nsew", pady=(10, 0))
        text_wrap.columnconfigure(0, weight=1)
        text_wrap.rowconfigure(0, weight=1)
        self.reader = self._reg(
            tk.Text(text_wrap, wrap="word", relief="flat", padx=18, pady=14,
        ...
        trans.grid(row=4, column=0, sticky="ew", pady=(10, 0))
    ...
    def _restyle_reader(self, save=False):
        ...
        self.reader.configure(bg=self.pal["CARD"], fg=self.pal["TEXT"],
                              font=self._reader_font(), spacing1=ls, spacing3=ls + 2)
