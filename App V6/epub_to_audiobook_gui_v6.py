"""
epub_to_audiobook_gui_v6.py — EPUB → MP3 Audiobook Converter (GUI, V6)
======================================================================
V4 plus Mandarin and Cantonese read-along:

  • CHINESE VOICES — a Language selector next to the voice menu offers Mandarin
    (Mainland zh-CN and Taiwan zh-TW) and Cantonese (Hong Kong zh-HK) narrators.
  • WORKS FOR CHINESE TEXT — sentence splitting now understands 。！？；…
    (Chinese has no spaces between sentences), part lengths use the Chinese
    speech rate (~300 chars/min), and counts/estimates switch to characters.
  • CHINESE READ-ALONG — highlighting comes straight from edge-tts's Chinese
    sentence timing (verified accurate), and the reader auto-switches to a
    Chinese font (PingFang SC/HK/TC) when a Chinese book is opened.
  • When a Chinese book is loaded, the voice is auto-set to Mandarin (switch to a
    Cantonese voice if you want Cantonese — the characters are the same).

Everything from V4 is unchanged: modern UI, Light/Dark, immersive text-first
read-along, real line spacing, page-turn scroll, speed/font/size/highlight,
auto-advance, resume, sleep timer, shortcuts. Built on `audiobook_engine_v6`.
...
import audiobook_engine_v6 as eng
...
# Chinese-capable fonts (macOS names; harmless if absent — Tk falls back).
CJK_FONTS = ["PingFang SC", "PingFang HK", "PingFang TC", "Heiti SC",
...
FONT_CHOICES = ["Georgia", "Palatino", "Iowan Old Style", "Charter",
                ...
                "Avenir Next", "Menlo", "Courier New"] + CJK_FONTS


def _cjk_font_for(voice: str) -> str:
    """A Chinese font matched to the voice's script (HK/TW = Traditional)."""
...
class App:
    ...
    def _build_convert_tab(self, parent):
        ...
        voice_wrap.columnconfigure(1, weight=1)
        # Language filter → repopulates the voice list (English / Mandarin / Cantonese).
        self.voice_var = tk.StringVar(value=self.settings.get("voice", eng.DEFAULT_VOICE))
        self.lang_var = tk.StringVar(value=self._lang_for_voice(self.voice_var.get()))
        self.lang_box = ttk.Combobox(voice_wrap, textvariable=self.lang_var, state="readonly",
                                     width=18, values=list(eng.VOICES_BY_LANG.keys()))
        self.lang_box.grid(row=0, column=0, padx=(0, 6))
        self.lang_box.bind("<<ComboboxSelected>>", self._on_lang_pick)
        self.voice_box = ttk.Combobox(voice_wrap, textvariable=self.voice_var,
                                      values=eng.VOICES_BY_LANG.get(self.lang_var.get(), eng.POPULAR_VOICES))
        self.voice_box.grid(row=0, column=1, sticky="ew")
    ...
    def _part_chars(self):
        try:
            ...
            minutes = 12
        # chars/min depends on the loaded book's language (Chinese is ~3× denser).
        return max(1000, minutes * getattr(self, "_cpm", eng.CHARS_PER_MIN))
    ...
    def _on_loaded(self, payload):
        ...
        # Detect language → drives part length, counts, and a voice suggestion.
        all_text = "".join(c["text"] for c in chapters)
        self._cjk = eng.is_cjk_text(all_text)
        self._cpm = eng.chars_per_min(all_text)
        count, est, unit = eng.estimate_words_minutes(chapters)
        njunk = sum(1 for c in chapters if c["junk"])

        note = ""
        if self._cjk and not self.voice_var.get().startswith("zh-"):
            # Pick a Mandarin voice by default (can't tell Mandarin vs Cantonese
            # from the characters — switch to a Cantonese voice if you want it).
            self._set_voice(eng.DEFAULT_VOICE_FOR_LANG["Mandarin (Mainland)"])
            note = "  ·  Chinese detected — voice set to Mandarin (pick a Cantonese voice if preferred)"

        self.info_var.set(f"📖  {title}\n👤  {author}\n"
                          f"{len(chapters)} chapters · ~{count:,} {unit} · "
    ...
    def _populate_chapters(self):
        ...
        for ch in self.chapters:
            ...
            tk.Label(row, text=f"~{wc:,} {unit} · {tag}", bg=self.pal["CARD"], fg=self.pal["MUTED"],
    ...
    def _update_sel_info(self):
        ...
        est = eng.estimate_words_minutes(sel)[1] if sel else 0
        self.sel_info.set(f"{len(sel)} of {len(self.chapters)} selected · ~{est // 60}h {est % 60}m")

    # ── voice / language ─────────────────────────────────────────────────────
    def _lang_for_voice(self, voice):
        for lang, voices in eng.VOICES_BY_LANG.items():
            if voice in voices:
                return lang
        if voice.startswith("zh-HK"):
            return "Cantonese (Hong Kong)"
        if voice.startswith("zh-TW"):
            return "Mandarin (Taiwan)"
        if voice.startswith("zh-"):
            return "Mandarin (Mainland)"
        return "English (US)"

    def _on_lang_pick(self, _evt=None):
        lang = self.lang_var.get()
        voices = eng.VOICES_BY_LANG.get(lang, eng.POPULAR_VOICES)
        self.voice_box.config(values=voices)
        if self.voice_var.get() not in voices:
            self.voice_var.set(voices[0])

    def _set_voice(self, voice):
        """Set the voice and sync the language selector to match."""
        self.voice_var.set(voice)
        self.lang_var.set(self._lang_for_voice(voice))
        self._on_lang_pick()
        self.voice_var.set(voice)
    ...
    def _load_track(self, path: Path):
        ...
        # Auto-switch to a Chinese font when the track is Chinese (so glyphs and
        # spacing look right), unless the user already picked a CJK font.
        if units:
            sample = "".join(u["text"] for u in units[:40])
            if eng.is_cjk_text(sample) and self.font_var.get() not in CJK_FONTS:
                self.font_var.set(_cjk_font_for(data.get("voice", "")))
