...
from audiobook_engine import (                      # noqa: F401
    CHARS_PER_MIN, CJK_CHARS_PER_MIN, DEFAULT_VOICE, POPULAR_VOICES,
    VOICES_BY_LANG, DEFAULT_VOICE_FOR_LANG, SUBS_SUFFIX,
    HAVE_MUTAGEN, missing_deps,
    extract_text, split_text, split_sentences,
    is_cjk_text, cjk_fraction, chars_per_min,
    ...
    extract_cover, write_tags, have_ffmpeg, ffmpeg_install_hint, combine_to_m4b,
...
def render_at_speed(mp3_path, speed: float) -> Path:
    ...
    if abs(speed - 1.0) < 1e-3:
        return mp3_path
    if not have_ffmpeg():
        raise RuntimeError("ffmpeg not found — needed for speed change.\n"
                           + ffmpeg_install_hint())
