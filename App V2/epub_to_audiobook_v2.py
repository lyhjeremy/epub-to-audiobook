"""
epub_to_audiobook_v2.py — EPUB → MP3 Audiobook Converter (command line, V2)
===========================================================================
Turns ANY .epub into a folder of nicely-named MP3 chapters that read the book
aloud word-for-word, using Microsoft Edge's free text-to-speech.

This V2 is a thin wrapper around the shared engine in `audiobook_engine.py`
(both this and the GUI import it, so there's one copy of the logic). On top of
everything V1 did, it adds:

    --subs / --no-subs   Write a sidecar "<mp3>.subs.json" of sentence timings
                         alongside each MP3 (default ON). The GUI uses these to
                         highlight the text in sync; harmless for plain listening.
    --skip-junk          Drop chapters auto-flagged as front/back matter
                         (cover, copyright, index, tiny pages …).
    --tag                Write ID3 tags + embedded cover onto each MP3 (mutagen).
    --m4b                After synthesis, also build one <Book>.m4b audiobook
                         with chapter markers + cover (needs ffmpeg on PATH).
    --preview-voice TXT  Speak a short sample with the chosen voice and exit.

SETUP (one-time)
----------------
    pip install edge-tts ebooklib beautifulsoup4 pygame
    pip install mutagen            # optional: --tag
    # ffmpeg on PATH               # optional: --m4b

USAGE
-----
    python epub_to_audiobook_v2.py path/to/book.epub
    python epub_to_audiobook_v2.py book.epub --list
    python epub_to_audiobook_v2.py book.epub --voice en-GB-SoniaNeural --announce
    python epub_to_audiobook_v2.py book.epub --skip-junk --m4b --tag

CRASH-SAFE
----------
Every finished file is written atomically and reused on the next run, so an
interrupted run (Ctrl+C, network drop, sleep) just resumes when re-run.
"""
...
# ── Friendly dependency check before importing the engine ─────────────────────
...
def _preview_voice(text, voice, rate):
    """Synthesise a short sample to a temp file and play it (needs pygame)."""
    import tempfile
    import os
    ...
    try:
        ...
        # No pygame / no audio device — fall back to a platform opener.
...
def main():
    parser = argparse.ArgumentParser(
        description="Convert any EPUB into a folder of nicely-named MP3 chapters (V2).",
    ...
    parser.add_argument("--voice", default=eng.DEFAULT_VOICE,
                        help=f"Edge TTS voice (default: {eng.DEFAULT_VOICE})")
    ...
    parser.add_argument("--skip-junk", action="store_true",
                        help="Skip chapters auto-flagged as front/back matter")
    ...
    parser.add_argument("--no-subs", dest="subs", action="store_false",
                        help="Don't write sidecar sentence-timing JSON files")
    parser.add_argument("--tag", action="store_true",
                        help="Write ID3 tags + cover onto each MP3 (needs mutagen)")
    parser.add_argument("--m4b", action="store_true",
                        help="Also build a single <Book>.m4b (needs ffmpeg)")
    parser.add_argument("--preview-voice", nargs="?", const=
                        ...
                        help="Speak a short sample with --voice and exit")
    ...
    part_chars = max(1000, int(args.part_minutes * eng.CHARS_PER_MIN))
    ...
    if args.skip_junk:
        ...
        if dropped:
            print(f"[*] --skip-junk dropped chapters: "
                  f"{', '.join(map(str, dropped))}\n")
    if not chapters:
        print("[X] No chapters found. Try --min-chars 100 to lower the threshold.")
        sys.exit(1)

    total_words, est_minutes = eng.estimate_words_minutes(chapters)
    ...
    for ch in chapters:
        ...
        print(f"    {ch['index']:>2}  {ttl[:52]:<52}  ~{wc:>6,} w  ({tag}){flag}")
    print()
    ...
    # ── optional post-processing: tags + .m4b ─────────────────────────────────
